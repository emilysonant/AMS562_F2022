...
void compute_n2e_adj(const unsigned n, const Triangles &conn,
  ...
  for(unsigned i = 0; i < conn.ntris(); i++) {
    ...
    for(int j = 0; j <= 2; j++) {
      int node = triangle[j];
      for(unsigned k = 0; k < conn.ntris(); k++) {
        std::array<int, 3> triangle2 = conn[k];
        for(unsigned l = 0; l <=2; l++) {
          int node2 = triangle2[l];
          /*if (std::find(adj[node].begin(), adj[node].end(), adj[node].size())); {
            break;
          } */
          if(node2 == node) {
            if(std::find(adj[node].begin(), adj[node].end(), adj[node].size()) = adj[node].end())){
              break;
            }
